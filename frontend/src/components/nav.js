...
const Nav = ({ homeRef, educationRef, workRef, portofolioRef}) => {
    ...
    <>
      <div className='hidden md:block sticky top-0'>
        <ul className='flex bg-stone-100 cursor-pointer'>
          <li
            className='mx-3 my-4 test-2xl test-slate-900 hover:text-slate-700 transition'
            onClick={() => executeScroll(homeRef)}
          >
            Test Portofolio
          </li>
          <li
            className='mx-3 my-5 test-slate-900 hover:text-slate-700 transition'
          ...
          <li
            className='mx-3 my-5 test-slate-900 hover:text-slate-700 transition'
            onClick={() => executeScroll(portofolioRef)}
          >
            Portofolio
          </li>
        </ul>
      ...
      <div className='md:hidden block sticky top-0'>
        <div className='flex justify-between w-100 bg-stone-100 cursor-pointer'>
          <div
            className='mx-3 my-4 test-2xl test-2xl test-slate-900 hover:text-slate-700 transition'
            onClick={() => executeScroll(homeRef)}
          >
            Test Portofolio
          </div>
        ...
        <div className={toggleMenu ? '' : 'hidden'}>
          <div className='grid grid-cols-1 bg-stone-100 cursor-pointer'>
            <div
              className='mx-3 my-5 text-slate-900 hover:text-slate-700 transition'
              onClick={() => executeScroll(educationRef)}
            >
              Education
            </div>
